...
#         self.publisher = rospy.Publisher('detections_3d', BoundingBox3D, queue_size=10)
...
#     def process_detections(self, img: Image, detections_msg: DetectionArray) -> List[BoundingBox3D]:
...
#             aux_msg = BoundingBox3D()
#             aux_msg.center = Pose()
#             aux_msg.size = Vector3()
...
#             # Calculate the center position (x, y, z) of the bounding box
#             # location[] typically contains the (x, y, z) world coordinates
#             aux_msg.center.position.x = location[0]
#             aux_msg.center.position.y = location[1]
#             aux_msg.center.position.z = location[2]

#             # Orientation of the bounding box
#             # Assuming we convert the orientation angle `alpha` (plus any necessary adjustments) to a quaternion:
#             quat = euler_to_quaternion(0, alpha, 0)  # You may need to adjust the roll, pitch, yaw values
#             aux_msg.center.orientation.x = quat[0]
#             aux_msg.center.orientation.y = quat[1]
#             aux_msg.center.orientation.z = quat[2]
#             aux_msg.center.orientation.w = quat[3]

#             # Dimensions of the bounding box (length, width, height)
#             aux_msg.size.x = dim[2]  # Assuming length is the third element
#             aux_msg.size.y = dim[1]  # width
#             aux_msg.size.z = dim[0]  # height
...
#         # rospy.loginfo("Received an image and detection array")
#         if self.enable and detections_msg.detections:
#             # new_detections_msg = DetectionArray()
#             # new_detections_msg.header = detections_msg.header
#             # new_detections_msg.detections.append(self.process_detections(
#             #     img, detections_msg))
#             # self.publisher.publish(new_detections_msg)
#             results = self.process_detections(img, detections_msg)
...
class Detection3DNode:
    def __init__(self):
        ...
        rospack = rospkg.RosPack()
        package_path = rospack.get_path('my_pkg_bev')
        self.model_path = os.path.join(package_path, 'model', 'mobilenetv2_weights.h5')
        self.device = rospy.get_param('~device', 'cpu')
        self.threshold = rospy.get_param('~threshold', 0.5)
        self.enable = rospy.get_param('~enable', True)
