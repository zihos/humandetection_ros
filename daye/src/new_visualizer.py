...
class Plot3DBoxBev:
    ...
    def __init__(
        ...
        self.fig = plt.figure(figsize=(80, 20), dpi=20)
...
def plot_3d_box(img, cam_to_img, ry, dimension, center):
    ...
    # Add distance_z text to the image
    text_position = (int(box_3d[0][0]), int(box_3d[0][1]) - 10)
    text = f'distance_z: {center[2]:.2f}'
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 0.5
    font_color = (255, 255, 255)
    font_thickness = 1

    cv2.putText(img, text, text_position, font, font_scale, font_color, font_thickness, lineType=cv2.LINE_AA)
...
# class Visualizer:
#     def __init__(self, is_bev_plot, is_img_w_3d, is_marker, shape, real_time):
...
#         # self.out = cv2.VideoWriter('mobilenet_output_video.mp4', fourcc, 15, (frame_width, frame_height))
...
#             # rospy.loginfo(f"{detections_3d_msgs.boxes}")
#             orient_reshaped = np.array(detections_3d_msg.orient).reshape(6, 2)
#             if self.BEV_plot:
#                 plot3d(img1, P2, detections_3d_msg.bbox, detections_3d_msg.dim, detections_3d_msg.alpha, detections_3d_msg.theta_ray)
...
#             plot3dbev.plot(img=img3, class_object=detections_3d_msg.classes, 
#                       bbox=[obj.xmin, obj.ymin, obj.xmax, obj.ymax],
#                       dim=[obj.h, obj.w, obj.l], loc=[obj.tx, obj.ty, obj.tz], rot_y=rot_y, objId=["pedestrian"] if not isinstance("pedestrian", list) else "pedestrian")
...
#             # if self.BEV_plot:
#             #     img3 = plot3dbev.plot(img=img3, class_object=obj.name.lower(), 
#             #         bbox=[obj.xmin, obj.ymin, obj.xmax, obj.ymax],
...
#                     marker.pose.position.y = detections_3d_msg.location[2] # 이미지 상에서 y축에서 걸어오는 것  처럼 보여야함. z로 하면 높이 차이가 됨. 
#                     marker.pose.position.z = detections_3d_msg.location[0]
...
#                     marker.scale.x = detections_3d_msg.dim[1] # shaft diameter =  height 
#                     marker.scale.y = detections_3d_msg.dim[0] # head diameter = length
...
#         if self.BEV_plot == "True":
#             plot3dbev.plot(img=img1)
...
class Visualizer:
    def __init__(self, is_bev_plot, is_img_w_3d, is_marker, shape, real_time):
        rospy.init_node('visualizer', anonymous=True)
        # message_filter를 사용하면 Rosbag을 통해서 image가 못받는 것 같음 
        # self.detection_3d_sub = message_filters.Subscriber("detections_3d",VisBoundingBox3DArray)
        self.detection_3d_sub = rospy.Subscriber("detections_3d",VisBoundingBox3DArray, self.detections_callback)
    ...
    def detections_callback(self, detections_3d_msg):
        self.current_detections_msg = detections_3d_msg
        self.process()
    ...
    # def process(self, detections_3d_msgs: VisBoundingBox3DArray, img:Image):
    def process(self):
        if self.current_detections_msg is not None and self.current_img is not None:
            rospy.loginfo('Processing detections and image')
            # for each frame of the video
            marker_array = MarkerArray()      
            plot3dbev = Plot3DBoxBev(P2)
            ...
            # Convert ROS Image message to OpenCV image
            try:
                img1 = self.cv_bridge.imgmsg_to_cv2(self.current_img, "bgr8") # for BEV 
                # img2 = self.cv_bridge.imgmsg_to_cv2(img, "bgr8") # for 2D-bb image 
                img3 = self.cv_bridge.imgmsg_to_cv2(self.current_img, "bgr8") # for 3D-bb image 
            except CvBridgeError as e:
                print(e)
                return []


            for i, detections_3d_msg in enumerate(self.current_detections_msg.boxes): # every detected-person in one image 
                # rospy.loginfo(f"{detections_3d_msgs.boxes}")
                orient_reshaped = np.array(detections_3d_msg.orient).reshape(6, 2)
                if self.BEV_plot == "True":
                    plot3d(img1, P2, detections_3d_msg.bbox, detections_3d_msg.dim, detections_3d_msg.alpha, detections_3d_msg.theta_ray)
                ...
                if self.is_marker == "True":
                    # Create a marker for RViz
                    marker = Marker()
                    marker.header = self.current_img.header 
                    # marker.header.frame_id = img.header.frame_id
                    marker.header.frame_id = "base_link"
                    # marker.header.frame_id = "camera_color_optical_frame"
                    marker.header.stamp = rospy.Time.now()
...
def get_args():
    ...
    parser.add_argument("--is_bev_plot",
                        type=str,
                        default="True",
                        help="Whether to visualize the BEV plot")
    parser.add_argument("--is_img_w_3d",
                        type=str,
                        default="False",
                        help="Wheter to visualize the 3D bounding box on the image")
    parser.add_argument("--is_marker",
                        type=str,
                        default="False",
                        help="Whether to visualize the cube marker array in rviz")
...
def main():
    ...
    # shape = "arrow" # "CUBE"
    shape = "CUBE" # "CUBE"
    node = Visualizer(opt.is_bev_plot, opt.is_img_w_3d, opt.is_marker, shape, opt.real_time)
