...
def broadcast_transform():
    ...
    #                      tf.transformations.quaternion_from_euler(0, -math.pi/2, 0),  # Rotation (roll, pitch, yaw) in radian
    #                      rospy.Time.now(),
    #                      "camera_link", # child frame, camera가 coordinates상에서 위로 image가 찍힌 곳처럼 위로 올라가 있어야 함
    #                      "base_link" # parent frame
    #                      )    
    while not rospy.is_shutdown():
        br.sendTransform((0.0, 0.0, 0.0),  # Translation (x, y, z)
                         ...
                         rospy.Time.now(),
                         "camera_color_optical_frame", # child frame, camera가 coordinates상에서 위로 image가 찍힌 곳처럼 위로 올라가 있어야 함
                         "base_link" # parent frame
                         )    
        rate.sleep()
