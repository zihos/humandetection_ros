...
class Plot3DBoxBev:
    ...
    def draw_3dbox(self, img, class_object, bbox, dim, loc, rot_y, distance_z):
        """draw 3d box"""
        ...
        # Display the distance (tz) on the image
        distance_text = f"Distance: {distance_z:.2f}"
        org = (int(corners_2D[:, 1][0]), int(corners_2D[:, 1][1]) - 10)  # Position to place the text
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = 0.5
        font_color = (0, 0, 255)  # Red color in BGR
        thickness = 1
        cv2.putText(img, distance_text, org, font, font_scale, font_color, thickness, cv2.LINE_AA)
...
class Visualizer:
    ...
    def callback(self, detections_3d_msgs: VisBoundingBox3DArray, img:Image):
        ...
        for i, detections_3d_msg in enumerate(detections_3d_msgs.boxes): # every detected-person in one image 
            ...
            img3, corners_3D = plot3dbev.draw_3dbox(img=img3, class_object=detections_3d_msg.classes, bbox=[obj.xmin, obj.ymin, obj.xmax, obj.ymax], dim=[obj.h, obj.w, obj.l], loc=[obj.tx, obj.ty, obj.tz], rot_y=rot_y, distance_z=obj.tz)
        ...
        # Display the image with 3d bounding box
        cv2.imshow("RGB and Depth Image", img3)
        ...
        if self.is_3d_img: 
            ...
            img_with_3d_bb = self.cv_bridge.cv2_to_imgmsg(img3, "bgr8")
